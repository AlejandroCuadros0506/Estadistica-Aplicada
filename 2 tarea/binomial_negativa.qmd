...
```{r}
#| label: exr-grafico-solucion-simple
#| code-fold: true
#| fig-cap: "Comparación de Densidades con Gráficos Base de R (r=4, p=0.3)"

comparar_distribuciones_simple <- function(r, p, n_sim) {
  
  # 1. Simulación de datos (esta parte es la misma)
  set.seed(42)
  sim_geometrica <- replicate(n_sim, sum(rgeom(r, prob = p)))
  sim_bn <- rnbinom(n_sim, size = r, prob = p)
  
  # 2. Imprimir un resumen directo en la consola
  cat("--- Resumen Estadístico de la Simulación ---\n")
  cat(sprintf("Media Suma Geométricas: %.4f\n", mean(sim_geometrica)))
  cat(sprintf("Media Binomial Negativa: %.4f\n", mean(sim_bn)))
  
  # 3. Gráfico de densidad usando funciones base de R (plot, lines, legend)
  # Esto es más simple y no necesita librerías adicionales.
  
  # Se dibujan las dos densidades superpuestas
  plot(density(sim_geometrica), 
       col = "blue", 
       lwd = 2, # Grosor de la línea
       main = "Comparación: Suma de Geométricas vs. Binomial Negativa",
       xlab = "Número de Fallas (k)",
       ylab = "Densidad")
  
  lines(density(sim_bn), col = "red", lty = 2, lwd = 2) # lty=2 es línea punteada
  
  # Añadir una leyenda para saber qué línea es cuál
  legend("topright", 
         legend = c("Suma de Geométricas", "Binomial Negativa"), 
         col = c("blue", "red"), 
         lty = c(1, 2), # 1=sólida, 2=punteada
         lwd = 2,
         bty = "n") # Sin caja alrededor de la leyenda
}

# --- Ejecutar la función simplificada ---
comparar_distribuciones_simple(r = 4, p = 0.3, n_sim = 10000)
```
...
```{r}
#| label: exr-baloncesto-solucion-alt
#| code-fold: true
#| fig-cap: "Frecuencia de Fallos en Tiros Libres (BN: r=8, p=0.7)"
...
# a) Modelación
cat("a) Modelo: X ~ Binomial Negativa(r=8, p=0.7) contando fallos.\n")

# b) Esperanza (sin cambios)
esperanza_fallos <- r2 * (1 - p2) / p2
cat(sprintf("b) Número esperado de fallos E(X): %.4f\n", esperanza_fallos))

# c) Simulación y comparación
set.seed(2)
fallos_simulados <- rnbinom(n_sim2, size = r2, prob = p2)
media_observada <- mean(fallos_simulados)
cat(sprintf("c) Media observada en simulación (n=%d): %.4f\n", n_sim2, media_observada))

# Gráfico de barras de frecuencia con anotación de texto
# Contar frecuencias de cada resultado
data_sim2 <- as.data.frame(table(fallos_simulados))
colnames(data_sim2) <- c("Fallas", "Frecuencia")
data_sim2$Fallas <- as.numeric(as.character(data_sim2$Fallas))

plot_alt2 <- ggplot(data_sim2, aes(x = Fallas, y = Frecuencia)) +
  geom_col(fill = "#0072B2", alpha = 0.8) + # <-- CAMBIO: Gráfico de barras
  # Anotación de texto para la media teórica
  geom_text(
    x = esperanza_fallos + 2, y = max(data_sim2$Frecuencia) * 0.8,
    label = paste("Media Teórica\nE(X) =", round(esperanza_fallos, 2)),
    color = "red", fontface = "bold"
  ) +
  geom_segment( # Flecha para señalar la media
    aes(x = esperanza_fallos + 1.5, y = max(data_sim2$Frecuencia) * 0.8, 
        xend = esperanza_fallos + 0.2, yend = max(data_sim2$Frecuencia) * 0.8),
    arrow = arrow(length = unit(0.2, "cm")), color = "red"
  ) +
  labs(
    title = "Frecuencia de Fallos en Tiros Libres",
    subtitle = paste("Media Observada:", round(media_observada, 2)),
...
```{r}
#| label: exr-biotecnologia-solucion-alt
#| code-fold: true
...
# Crear un Data Frame para una salida ordenada
resultados <- data.frame(
  Descripcion = c(
    "Probabilidad de exactamente 12 fallas P(X=12)",
    "Probabilidad de más de 20 fallas P(X>20)",
    "Número esperado de fallas E(X)",
    "Costo esperado de las fallas"
  ),
  Valor = c(
    round(prob_a3, 4),
    round(prob_b3, 4),
    round(esperanza_fallos_c, 2),
    scales::dollar(costo_esperado)
  )
)

# Imprimir la tabla de resultados
cat("--- Resumen del Análisis de Pruebas de Biotecnología ---\n")
