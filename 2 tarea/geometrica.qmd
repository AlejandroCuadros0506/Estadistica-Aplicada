...
-   
```{r}
#| label: exr-geom_1-solucion-simple
#| code-fold: true

# --- Función para Analizar la Distribución Geométrica ---
# Esta función toma la probabilidad de éxito (p) y realiza los cálculos.
analizar_lanzamientos_moneda <- function(p) {
  
  # a) P(X=5) -> 4 fallos antes del 1er éxito
  prob_5_lanzamientos <- dgeom(x = 4, prob = p)
  
  # b) P(X > 3) -> 1 - P(X <= 3) -> 1 - P(hasta 2 fallos)
  prob_mas_de_3 <- 1 - pgeom(q = 2, prob = p)
  
  # c) Media y Varianza del número de lanzamientos
  media_lanzamientos <- 1 / p
  varianza_lanzamientos <- (1 - p) / (p^2)
  
  # Organiza los resultados en una lista
  resultados <- list(
    "P(X=5 lanzamientos)" = round(prob_5_lanzamientos, 4),
    "P(X>3 lanzamientos)" = round(prob_mas_de_3, 4),
    "Media_E(X)" = media_lanzamientos,
    "Varianza_Var(X)" = varianza_lanzamientos
  )
...
prob_exito <- 0.5
resultados_moneda <- analizar_lanzamientos_moneda(p = prob_exito)

# Imprime la lista de resultados de forma clara
cat("--- Resultados del Ejercicio de la Moneda ---\n")
print(as.data.frame(resultados_moneda))
```
...
-   
```{r}
#| label: exr-geom_2-solucion-simple
#| code-fold: true

# --- Función para Analizar la Distribución Geométrica en Tiros Libres ---
# Toma la probabilidad de éxito (p) y calcula las métricas solicitadas.
analizar_tiros_libres <- function(p) {
  
  # a) P(X=3) -> 2 fallos antes del 1er éxito
  prob_3_intentos <- dgeom(x = 2, prob = p)
  
  # b) Número esperado de intentos E(X) = 1/p
  intentos_esperados <- 1 / p
  
  # c) P(2 <= X <= 5) = P(X <= 5) - P(X < 2)
  # P(X <= 5) es la probabilidad de tener hasta 4 fallos.
  # P(X < 2) es la probabilidad de tener hasta 0 fallos (P(X=1)).
  prob_entre_2_y_5 <- pgeom(q = 4, prob = p) - pgeom(q = 0, prob = p)
  
  # Organiza los resultados en una lista para una salida limpia
  resultados <- list(
    "P(X=3 intentos)" = round(prob_3_intentos, 4),
    "Intentos_esperados_E(X)" = round(intentos_esperados, 4),
    "P(2<=X<=5 intentos)" = round(prob_entre_2_y_5, 4)
  )
...
prob_anotar <- 0.7
resultados_baloncesto <- analizar_tiros_libres(p = prob_anotar)

# Imprime la lista de resultados como una tabla (data.frame)
cat("--- Resultados del Ejercicio de Baloncesto ---\n")
print(as.data.frame(resultados_baloncesto))
```
...
-   
```{r}
#| label: exr-geom_3-solucion-simple
#| code-fold: true

# --- Función para analizar la inspección de la máquina ---
# Realiza todos los cálculos y presenta los resultados de forma ordenada.
analizar_inspeccion_maquina <- function(p) {
  
  # a) Número esperado de inspecciones E(X) = 1/p
  esperanza <- 1 / p
  
  # b) P(X > 100), que es la probabilidad de tener 100 o más fallos
  prob_mayor_100 <- 1 - pgeom(q = 99, prob = p)
  
  # c) Demostración de la Falta de Memoria
  # P(X > 100) calculado con la fórmula (1-p)^k
  p_directo <- (1 - p)^100
  # P(X > 110 | X > 10)
  p_condicional <- (1 - p)^(110) / (1 - p)^10
  
  # --- Presentación de Resultados ---
  cat("--- Análisis de la Máquina Defectuosa ---\n\n")
  
  cat(sprintf("a) Número esperado de inspecciones E(X): %.2f\n", esperanza))
  cat(sprintf("b) Probabilidad de necesitar más de 100 inspecciones P(X>100): %.4f\n", prob_mayor_100))
  
  cat("\nc) Demostración de la Falta de Memoria:\n")
  cat("   P(X > 110 | X > 10) debe ser igual a P(X > 100)\n")
  cat(sprintf("   - El cálculo condicional da: %.4f\n", p_condicional))
  cat(sprintf("   - El cálculo directo da:    %.4f\n", p_directo))
  cat("   Como los resultados son idénticos, la propiedad se cumple. ✅\n")
}

# --- Ejecutar la función con la probabilidad dada ---
prob_defecto <- 0.02
analizar_inspeccion_maquina(p = prob_defecto)
```
