...
```{r}
#| label: exr-tcl_poisson-solucion-simple
#| code-fold: true
#| fig-cap: "Verificación del TCL para Poisson(7) con Gráficos Base de R"

# --- Función para demostrar el TCL con una distribución Poisson ---
demostrar_tcl_poisson <- function(lambda, n, k_sim) {
  
  # 1. Simular k_sim medias muestrales de tamaño n
  set.seed(456)
  medias_muestrales <- replicate(k_sim, mean(rpois(n, lambda = lambda)))
  
  # 2. Parámetros teóricos de la distribución de la media muestral
  # Según el TCL, la media es μ y la desviación estándar es σ/√n
  # Para Poisson: μ = λ y σ² = λ
  media_teorica <- lambda
  sd_teorica <- sqrt(lambda / n)
  
  # 3. Gráfico con funciones base de R (más simple que ggplot2)
  # Histograma de las medias muestrales
  hist(medias_muestrales, 
       freq = FALSE, # Muestra densidad en lugar de frecuencia
       breaks = 30, 
       main = "TCL para la Distribución de Poisson",
       xlab = "Media Muestral",
       col = "skyblue")
  ...
  curve(dnorm(x, mean = media_teorica, sd = sd_teorica), 
        ...
        add = TRUE) # 'add=TRUE' dibuja sobre el gráfico existente
        
  # Añadir leyenda
  legend("topright", 
         legend = "Normal Teórica (TCL)", 
         col = "blue", 
         lwd = 2, 
         bty = "n")

  # 4. Comparar estadísticas
  cat("--- Comparación Teórica vs. Simulación ---\n")
  cat(sprintf("Media Teórica (λ):         %.4f\n", media_teorica))
  cat(sprintf("Media Observada (sim):   %.4f\n\n", mean(medias_muestrales)))
  
  cat(sprintf("Varianza Teórica (λ/n):    %.4f\n", sd_teorica^2))
  cat(sprintf("Varianza Observada (sim):  %.4f\n", var(medias_muestrales)))
}

# --- Ejecutar la función ---
demostrar_tcl_poisson(lambda = 7, n = 30, k_sim = 1000)
```
...
```{r}
#| label: exr-prop_aditiva-solucion-simple
#| code-fold: true

# --- Función para demostrar la propiedad aditiva de Poisson ---
# Simula y compara la suma de variables Poisson con su equivalente teórico.
demostrar_aditividad_poisson <- function(lambdas, n_sim) {
  
  set.seed(123) # Para resultados reproducibles
  
  # --- 1. Verificación para la suma de las primeras dos variables ---
  cat("--- Verificación para Z = X1 + X2 ---\n")
  
  # Simulación
  X1 <- rpois(n_sim, lambda = lambdas[1])
  X2 <- rpois(n_sim, lambda = lambdas[2])
  Z <- X1 + X2
  
  # Comparación teórica vs. observada
  lambda_z_teorico <- lambdas[1] + lambdas[2]
  
  cat(sprintf("   Lambda Teórico (λ1 + λ2):    %.2f\n", lambda_z_teorico))
  cat(sprintf("   Media Observada de Z:      %.4f\n", mean(Z)))
  cat(sprintf("   Varianza Observada de Z:   %.4f\n", var(Z)))
  cat("   (Los valores observados son muy cercanos al teórico) ✅\n\n")
  
  # --- 2. Extensión a la suma de todas las variables ---
  cat("--- Extensión para W = X1 + X2 + X3 + X4 ---\n")

  # Simulación de las variables adicionales
  X3 <- rpois(n_sim, lambda = lambdas[3])
  X4 <- rpois(n_sim, lambda = lambdas[4])
  W <- Z + X3 + X4 # Reutilizamos la suma anterior
  
  # Comparación teórica vs. observada
  lambda_w_teorico <- sum(lambdas)

  cat(sprintf("   Lambda Teórico (Σλ):       %.2f\n", lambda_w_teorico))
  cat(sprintf("   Media Observada de W:      %.4f\n", mean(W)))
  cat("   (La media observada se aproxima al lambda total) ✅\n")
}

# --- Parámetros de la simulación ---
# Vector con todos los lambdas a utilizar
lambdas_a_probar <- c(3.5, 2.8, 1.0, 1.5)
numero_simulaciones <- 10000

# --- Ejecutar la función ---
demostrar_aditividad_poisson(lambdas = lambdas_a_probar, n_sim = numero_simulaciones)
```
...
```{r}
#| label: exr-aplicacion_web-solucion-simple
#| code-fold: true

# --- Función para analizar el rendimiento de la aplicación web ---
# Genera un informe completo basado en la tasa de visitantes.
analizar_trafico_web <- function(tasa_visitantes) {
  
  cat("--- Análisis de Carga de la Aplicación Web ---\n")
  cat(sprintf("Tasa base (λ): %d visitantes por minuto\n\n", tasa_visitantes))
  
  # a) Probabilidad de degradación con capacidad actual de 25
  capacidad_actual <- 25
  prob_degradacion_actual <- 1 - ppois(q = capacidad_actual, lambda = tasa_visitantes)
  cat(sprintf("a) Probabilidad de degradación (si X > %d): %.4f\n", capacidad_actual, prob_degradacion_actual))
  
  # b) Probabilidad de degradación si se duplica la capacidad a 50
  capacidad_duplicada <- 50
  prob_degradacion_nueva <- 1 - ppois(q = capacidad_duplicada, lambda = tasa_visitantes)
  cat(sprintf("b) Probabilidad de degradación (si X > %d): %.4f (prácticamente cero)\n", capacidad_duplicada, prob_degradacion_nueva))

  # c) Capacidad necesaria para que la probabilidad de degradación sea < 1%
  # Buscamos k tal que P(X > k) < 0.01, lo que es igual a P(X <= k) > 0.99
  capacidad_optima <- qpois(p = 0.99, lambda = tasa_visitantes)
  cat(sprintf("c) Capacidad óptima para P(degradación) < 1%%: %d visitantes\n", capacidad_optima))
  
  # d) Simulación de 1 hora (60 minutos) con la capacidad actual
  set.seed(789)
  visitas_por_minuto <- rpois(n = 60, lambda = tasa_visitantes)
  minutos_con_problemas <- sum(visitas_por_minuto > capacidad_actual)
  porcentaje_problemas <- minutos_con_problemas / 60
  
  cat(sprintf("d) Simulación de 1 hora: Hubo degradación en %d de 60 minutos (%.2f%% del tiempo).\n", 
              minutos_con_problemas, porcentaje_problemas * 100))
}

# --- Ejecutar el análisis ---
analizar_trafico_web(tasa_visitantes = 15)
```
...
```{r}
#| label: exr-inventarios-solucion-simple
#| code-fold: true
#| fig-cap: "Costo Esperado Diario vs. Nivel de Inventario (Q)"

# --- Función para el Análisis Completo de Inventarios ---
# Realiza todos los cálculos y presenta un informe y un gráfico.
analizar_inventario_poisson <- function(lambda, costo_mantener, costo_perdida, costo_adquisicion) {

  # --- Función interna para calcular el costo total esperado para un nivel Q ---
  calcular_costo_esperado <- function(Q) {
    # Se considera un rango amplio de posibles demandas diarias (x)
    max_demanda <- qpois(0.9999, lambda)
    posible_demanda <- 0:max_demanda
    probabilidades <- dpois(posible_demanda, lambda)
    
    # 1. Costo de adquisición (fijo para un Q dado)
    costo_compra <- Q * costo_adquisicion
    
    # 2. Costo esperado por mantener inventario sobrante
    # Se calcula como: costo_mantener * E[max(0, Q - Demanda)]
    sobrante <- pmax(0, Q - posible_demanda)
    costo_esperado_mantener <- sum(costo_mantener * sobrante * probabilidades)
    
    # 3. Costo esperado por ventas perdidas (faltante)
    # Se calcula como: costo_perdida * E[max(0, Demanda - Q)]
    faltante <- pmax(0, posible_demanda - Q)
    costo_esperado_perdida <- sum(costo_perdida * faltante * probabilidades)
    
    return(costo_compra + costo_esperado_mantener + costo_esperado_perdida)
  }

  # --- Inicio del Informe ---
  cat("--- Análisis de Política de Inventarios ---\n")
  cat(sprintf("Parámetros: λ=%.1f, C_mantener=$%.2f, C_perdida=$%.2f, C_adquisicion=$%.2f\n\n",
              lambda, costo_mantener, costo_perdida, costo_adquisicion))

  # a) Probabilidad de agotarse con Q=12
  prob_agotarse_12 <- 1 - ppois(q = 12, lambda = lambda)
  cat(sprintf("a) Probabilidad de agotarse con 12 unidades: %.4f\n", prob_agotarse_12))

  # b) Unidades necesarias para P(agotarse) < 5%
  unidades_95_conf <- qpois(p = 0.95, lambda = lambda)
  cat(sprintf("b) Unidades para P(agotarse) < 5%%: %d unidades\n", unidades_95_conf))

  # c) Costo esperado para Q=12
  costo_q12 <- calcular_costo_esperado(Q = 12)
  cat(sprintf("c) Costo esperado total si Q=12: $%.2f\n", costo_q12))

  # d) Nivel óptimo de inventario
  rango_q <- 1:20 # Rango razonable a evaluar
  costos <- sapply(rango_q, calcular_costo_esperado)
  
  costo_minimo <- min(costos)
  q_optimo <- rango_q[which.min(costos)]
  
  cat(sprintf("d) Nivel óptimo de inventario (Q*): %d unidades, con un costo mínimo de $%.2f\n",
              q_optimo, costo_minimo))

  # --- Gráfico de Costos (usando R base) ---
  plot(rango_q, costos, 
       type = "b", # 'b' para puntos y líneas
       col = "blue", 
       pch = 19,   # Círculos sólidos
       main = "Costo Esperado Diario vs. Nivel de Inventario (Q)",
       xlab = "Unidades en Inventario (Q)",
       ylab = "Costo Total Esperado ($)")
  
  # Resaltar el punto óptimo en el gráfico
  points(q_optimo, costo_minimo, col = "red", pch = 19, cex = 2)
  legend("topright", legend = "Costo Mínimo (Óptimo)", col = "red", pch = 19, bty = "n")
}

# --- Ejecutar el análisis completo ---
analizar_inventario_poisson(lambda = 8, costo_mantener = 2, costo_perdida = 15, costo_adquisicion = 10)
```
