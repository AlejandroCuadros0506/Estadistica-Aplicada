...
```{r}
#| label: exr-t-f-relacion-tidy
#| code-fold: true
#| fig-cap: "Relación entre la distribución t de Student al cuadrado y la 
# --- 1. Parámetros de la simulación ---
n_grados_libertad <- 15
num_simulaciones <- 20000

# --- 2. Simulación y Creación del Data Frame en un solo paso ---
set.seed(123) # Para reproducibilidad

datos_simulacion <- tibble(
  # Generar muestra de una distribución t(n)
  muestra_t = rt(num_simulaciones, df = n_grados_libertad)
) %>%
  # Calcular la variable al cuadrado en una nueva columna
  mutate(
    muestra_t_cuadrada = muestra_t^2
  )

# --- 3. Visualización y Comparación ---
ggplot(datos_simulacion, aes(x = muestra_t_cuadrada)) +
  geom_histogram(aes(y = after_stat(density)), bins = 50,
                 fill = "skyblue", color = "black", alpha = 0.7) +
  # Superponer la PDF teórica de F(1, n)
  stat_function(
    fun = df,
  ...
  labs(
    title = "Verificación: Si X ~ t(n), entonces X² ~ F(1, n)",
...
#| label: exr-f-reciproca-tidy
#| code-fold: true
#| fig-cap: "Relación recíproca de la distribución F"
...
datos_simulacion <- tibble(
  # Generar muestra de una distribución F(a, b)
  muestra_F_ab = rf(num_simulaciones, df1 = a, df2 = b)
) %>%
  # Calcular el recíproco en una nueva columna
  mutate(
    muestra_reciproca = 1 / muestra_F_ab
  )

# --- 3. Visualización y Comparación ---
ggplot(datos_simulacion, aes(x = muestra_reciproca)) +
  geom_histogram(aes(y = after_stat(density)), bins = 50,
                 fill = "lightgreen", color = "black", alpha = 0.7) +
  # Superponer la PDF teórica de F(b, a)
  stat_function(
    fun = df,
  ...
  labs(
    title = "Verificación: Si X ~ F(a, b), entonces 1/X ~ F(b, a)",
    subtitle = paste("Simulación para F(", a, ",", b, "). La curva azul es la PDF de F(", b, ",", a, ")."),
    x = "Valor de 1/X",
    y = "Densidad"
  ) +
  theme_minimal() +
  coord_cartesian(xlim = c(0, 5)) # Acotar el eje X para una mejor visualización
```


c) Si $X \chi^2(a)$ y $Y \sim \chi^2(b)$ son independientes, entonces $\frac{X/a}{Y/b} \sim F(a,b)$.
```{r}
#| label: exr-fisher-c-tidy
#| code-fold: true
#| warning: false
...
# --- 2. Simulación y Creación del Data Frame en un solo bloque ---
set.seed(789) # Para reproducibilidad

datos_simulacion <- tibble(
  # Generar muestras independientes de dos distribuciones Chi-cuadrada
  X = rchisq(num_simulaciones, df = a),
  Y = rchisq(num_simulaciones, df = b)
) %>%
  # Calcular la variable F según la fórmula en una nueva columna
  mutate(
    muestra_F_calculada = (X / a) / (Y / b)
  )

# --- 3. Visualización y Comparación ---
ggplot(datos_simulacion, aes(x = muestra_F_calculada)) +
  geom_histogram(aes(y = after_stat(density)), bins = 50,
                 fill = "orange", color = "black", alpha = 0.7) +
  # Superponer la PDF teórica de F(a, b)
  stat_function(
    fun = df,
