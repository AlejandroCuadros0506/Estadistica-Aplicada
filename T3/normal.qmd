...
```{r}
#| label: exr-normal-estaturas-stats-tidy
#| code-fold: true
...
# --- 2. Simulación y Estadísticos (Enfoque Tidyverse) ---
set.seed(42) # Añadido para reproducibilidad

# Generamos los datos y calculamos los estadísticos en un solo flujo
sample_summary <- tibble(
  altura = rnorm(n_sample, mean = pop_mean, sd = pop_sd)
) %>%
  summarise(
    Media = mean(altura),
    Mediana = median(altura),
    `Desviación Estándar` = sd(altura)
  ) %>%
  # Reorganizamos la tabla para que sea vertical
  pivot_longer(
    cols = everything(),
...
cat("Resumen de la Muestra de Alturas (n=", n_sample, "):\n", sep = "")

# Usamos kable para una presentación más limpia
kable(sample_summary, digits = 4, caption = "Estadísticos de la Muestra")
```
```{r}
# --- 1. Parámetros de la Curva ---
# (Asumimos que existen del bloque anterior)
mu_altura <- 170
sigma_altura <- 8
cutoff_val <- 180 # Definimos el punto de corte

# --- 2. Crear Data Frame para la Curva Teórica ---
# (Tu código original está perfecto, solo corregí el typo 'altrua')
dist_df <- tibble(
  estatura = seq(mu_altura - 4 * sigma_altura, mu_altura + 4 * sigma_altura, length.out = 1000),
  pdf = dnorm(estatura, mean = mu_altura, sd = sigma_altura),
  cdf = pnorm(estatura, mean = mu_altura, sd = sigma_altura) # Corregido typo
)

# --- 3. Calcular Probabilidad para la Anotación ---
prob_valor <- 1 - pnorm(cutoff_val, mu_altura, sigma_altura)

# --- 4. Visualización con Estética Renovada ---
ggplot(dist_df, aes(x = estatura, y = pdf)) +
  
  # Capa 1: La curva de densidad principal
  geom_line(color = "#0072B2", linewidth = 1.1) + # Tono azul
  
  # Capa 2: El área sombreada (usando un filtro de dplyr)
  geom_area(
    data = . %>% filter(estatura >= cutoff_val),
    fill = "#D55E00", # Tono naranja
    alpha = 0.7
  ) +
  
  # Capa 3: Línea vertical en el punto de corte
  geom_vline(xintercept = cutoff_val, linetype = "dotted", color = "black", linewidth = 1) +
  
  # Capa 4: Anotación (con estilo diferente y 'parse = TRUE')
  annotate(
    "label",
    x = 188,
    y = 0.015,
    # Usamos paste() con parse = TRUE para un formato matemático
    label = paste("P(X >= ", cutoff_val, ") ==", round(prob_valor, 4)),
    parse = TRUE,
    fill = "white",
    color = "#D55E00", # Color del texto a juego con el área
    size = 4
  ) +
  
  # Capa 5: Etiquetas y Tema
  labs(
    title = "Área bajo la Curva Normal",
    subtitle = bquote("Probabilidad de que la estatura sea" >= .(cutoff_val) ~ "cm"),
    x = "Estatura (cm)",
    y = "Densidad"
  ) +
  theme_classic() + # Nuevo tema
  theme(
...
```{r}
sim_df <- data.frame(estatura = muestra_altura)

ggplot(sim_df, aes(x = estatura)) +
  
  # Capa 1: Histograma de la simulación
  geom_histogram(
    aes(y = after_stat(density)),
    fill = "#009E73",  # Un verde azulado (colorblind-friendly)
    color = "white",   # Borde blanco para las barras
    alpha = 0.8,       # Ligera transparencia
    bins = 30          # Definir el número de barras (más común que 'center')
  ) +
  
  # Capa 2: Línea de la densidad teórica (usando el otro data frame)
  geom_line(
    data = dist_altura, 
    aes(x = estaturas, y = pdf_altura),
    color = "#D55E00",  # Un naranja oscuro
    linewidth = 1.2    # Grosor de la línea
  ) +
  
  # Capa 3: Etiquetas y Títulos
  labs(
    title = "Histograma de Simulación vs. Densidad Teórica",
    # bquote() permite formato matemático e insertar variables
    subtitle = bquote("Simulación (n=" ~ .(length(muestra_altura)) ~ ") de" ~ N(mu==.(mu_altura), sigma==.(sigma_altura))),
    x = "Estatura (cm)",
    y = "Densidad"
  ) +
  
  # Capa 4: Tema
  theme_light() + # Un tema limpio y moderno
  theme(
    plot.title = element_text(hjust = 0.5, face = "bold"),
    plot.subtitle = element_text(hjust = 0.5)
  )
...
mu_centered <- 350
sigma_current <- 5
LSL <- 340 # Límite Inferior (Lower Spec Limit)
USL <- 360 # Límite Superior (Upper Spec Limit)
n_sims <- 1000

# Parámetros de los escenarios
mu_shifted <- 355
target_quality_pct <- 99.9

# --- 2. Simulación (como en el original) ---
set.seed(456)
mediciones_temp <- rnorm(n_sims, mean = mu_centered, sd = sigma_current)
cat(paste("Se han simulado", n_sims, "mediciones de temperatura.\n\n"))

# --- 3. Análisis Teórico del Proceso (en una Tabla) ---
cat("--- Análisis Teórico de Capacidad del Proceso ---\n")

# -- Cálculos para la tabla --

# Escenario 1: Proceso Centrado
prob_fuera_centrado <- (
  pnorm(LSL, mean = mu_centered, sd = sigma_current) +
  pnorm(USL, mean = mu_centered, sd = sigma_current, lower.tail = FALSE)
)

# Escenario 2: Proceso Descentrado
prob_fuera_descentrado <- (
  pnorm(LSL, mean = mu_shifted, sd = sigma_current) +
  pnorm(USL, mean = mu_shifted, sd = sigma_current, lower.tail = FALSE)
)

# Escenario 3: Mejora del Proceso (Sigma necesaria)
# 99.9% dentro -> 0.1% fuera -> 0.05% en cada cola (0.0005)
target_z <- qnorm(1 - ( (100 - target_quality_pct) / 100 / 2) ) # qnorm(0.9995)
sigma_necesario <- (USL - mu_centered) / target_z

# -- Creación del Tibble de Resumen --
analysis_summary <- tibble(
  `Escenario` = c(
    "Proceso Centrado (μ = 350)",
    "Proceso Descentrado (μ = 355)",
    "Mejora del Proceso"
  ),
  `Métrica` = c(
    "Pct. Fuera de Especificación [340, 360]",
    "Pct. Fuera de Especificación [340, 360]",
    paste("Sigma requerida para", target_quality_pct, "% de calidad")
  ),
  `Valor Calculado` = c(
    prob_fuera_centrado * 100,
    prob_fuera_descentrado * 100,
    sigma_necesario
  ),
  `Unidad` = c("%", "%", "°C")
)
...
kable(analysis_summary, 
      digits = 4, 
      caption = "Análisis de Calidad del Proceso de Temperatura")
```
...
# --- 2. Análisis Teórico (Reemplazando cat()) ---
cat("--- Análisis Teórico de Ventas Mensuales ---\n")

# Creamos un tibble con todos los cálculos teóricos
theoretical_summary <- tibble(
  `Métrica` = c(
    "Probabilidad de ventas > $60,000",
    "Meses esperados (de 24) con ventas < $30,000",
    "Rango Intercuartílico (IQR) Teórico",
    "Porcentaje de meses que cumplen meta > $45,000"
  ),
  `Valor Calculado` = c(
    round(pnorm(60000, mu_ventas, sigma_ventas, lower.tail = FALSE), 4),
    round(pnorm(30000, mu_ventas, sigma_ventas) * n_meses, 1),
    round(qnorm(0.75, mu_ventas, sigma_ventas) - qnorm(0.25, mu_ventas, sigma_ventas), 2),
    round(pnorm(45000, mu_ventas, sigma_ventas, lower.tail = FALSE) * 100, 2)
  ),
  `Unidad` = c("Probabilidad", "Meses", "Pesos", "%")
)

# Mostramos la tabla teórica
kable(theoretical_summary, caption = "Análisis Teórico de Ventas Mensuales")
cat("\n")

# --- 3. Simulación de 24 Meses (en un Tibble) ---
set.seed(789)
ventas_simuladas_df <- tibble(
  Mes = 1:n_meses,
  Ventas_Simuladas = round(rnorm(n_meses, mean = mu_ventas, sd = sigma_ventas), 2)
)

cat("--- Simulación de 24 Meses ---\n")
# Imprimir el tibble completo
print(ventas_simuladas_df, n = n_meses)
```
...
:::
```{r}
#| label: exr-normal-baterias-tidy
#| code-fold: true
...
# --- 2. Análisis Teórico (Garantías y Percentiles) ---
cat("--- Análisis Teórico de la Vida Útil de las Baterías ---\n")

# Creamos un tibble con todos los cálculos teóricos
theoretical_summary <- tibble(
  `Métrica` = c(
    "Garantía para cubrir el 95% (Percentil 5)",
    "Baterías que fallan por cada 10,000 (Garantía de 900h)",
    "Percentil 10 de duración"
  ),
  `Valor Calculado` = c(
    qnorm(0.05, mean = mu_bateria, sd = sigma_bateria),
    pnorm(900, mean = mu_bateria, sd = sigma_bateria) * 10000,
    qnorm(0.10, mean = mu_bateria, sd = sigma_bateria)
  ),
  `Unidad` = c("Horas", "Baterías", "Horas")
)

# Mostramos la tabla teórica
kable(theoretical_summary, 
      digits = c(0, 1, 1, 2), # Controlamos los decimales por columna
      caption = "Cálculos Teóricos sobre la Garantía")
cat("\n")

# --- 3. Simulación y Comparación de Parámetros ---
set.seed(101)
cat(paste("--- Comparación de Parámetros (Simulación n=", n_sims, ") ---\n", sep = ""))

# Generamos datos y resumimos en un solo flujo
simulation_summary <- tibble(
  vida_util = rnorm(n_sims, mean = mu_bateria, sd = sigma_bateria)
) %>%
  summarise(
    Media = mean(vida_util),
    Mediana = median(vida_util)
  ) %>%
  # Pivotamos la tabla para una mejor comparación
  pivot_longer(cols = everything(), names_to = "Estadístico", values_to = "Valor Simulado") %>%
  mutate(
    `Valor Teórico` = mu_bateria # La media y mediana teóricas son iguales
  )

# Mostramos la tabla de simetría
kable(simulation_summary, 
      digits = 2, 
      caption = "Comparación de Parámetros: Simulación vs. Teórico")
```
...
# --- 2. Cálculos Teóricos Organizados en una Tabla ---
cat("--- Análisis de Precisión Teórico ---\n")

# Creamos un tibble con todos los cálculos teóricos
theoretical_summary <- tibble(
  `Métrica` = c(
    "Pct. de errores entre -5 y +5 (μ ± 2σ)",
    "Prob. de error |X| > 6 (extremo)",
    "Pct. de mediciones 'precisas' (|X| ≤ 3)"
  ),
  `Probabilidad` = c(
    pnorm(5, mu_error, sigma_error) - pnorm(-5, mu_error, sigma_error),
    pnorm(6, mu_error, sigma_error, lower.tail = FALSE) + pnorm(-6, mu_error, sigma_error),
    pnorm(3, mu_error, sigma_error) - pnorm(-3, mu_error, sigma_error)
  )
) %>%
  mutate(
...
kable(theoretical_summary, 
      digits = 4, 
      caption = "Análisis Teórico de Errores de Medición")
cat("\n")

# --- 3. Simulación y Verificación de Simetría ---
set.seed(202)
cat(paste("Se han simulado", n_sims, "errores de medición.\n"))
cat("--- Verificación de Simetría (Simulación) ---\n")

# Generamos datos y resumimos en un solo flujo
simulation_summary <- tibble(
  error_valor = rnorm(n_sims, mean = mu_error, sd = sigma_error)
) %>%
  summarise(
    `Media Simulada` = mean(error_valor),
    `Mediana Simulada` = median(error_valor)
  ) %>%
  # Pivotamos la tabla para una mejor comparación
  pivot_longer(cols = everything(), names_to = "Estadístico", values_to = "Valor Simulado") %>%
  mutate(
    `Valor Teórico` = 0 # La media y mediana teóricas son 0
  )

# Mostramos la tabla de simetría
kable(simulation_summary, 
      digits = 4, 
      caption = "Simetría: Comparación de Media y Mediana")
```
...
```{r}
#| label: exr-normal-comparacion-tidy
#| code-fold: true
...
# --- 1. Parámetros de los Grupos ---
mean_A <- 75; sd_A <- 10
mean_B <- 82; sd_B <- 8
n_students <- 200

# --- 2. Cálculos Teóricos y Resumen en Tabla ---
theoretical_summary <- tibble(
  `Métrica` = c(
    "Pct. Grupo A > Media de B (82 pts)",
    "Prob. Grupo B < 70 pts",
    "Corte Top 10% (P90) - Grupo A",
    "Corte Top 10% (P90) - Grupo B"
  ),
  `Valor` = c(
    pnorm(mean_B, mean = mean_A, sd = sd_A, lower.tail = FALSE) * 100,
    pnorm(70, mean = mean_B, sd = sd_B) * 100,
    qnorm(0.90, mean = mean_A, sd = sd_A),
    qnorm(0.90, mean = mean_B, sd = sd_B)
  ),
  `Unidad` = c("%", "%", "puntos", "puntos")
)

cat("--- Resumen de Comparaciones Teóricas ---\n")
kable(theoretical_summary, digits = 2, caption = "Análisis Teórico de Puntuaciones")
cat("\n")

# --- 3. Simulación (Formato Tidyverse) ---
set.seed(303)
simulation_df <- tibble(
  # Generar las simulaciones directamente en columnas
  `Grupo A` = rnorm(n_students, mean = mean_A, sd = sd_A),
  `Grupo B` = rnorm(n_students, mean = mean_B, sd = sd_B)
) %>%
  # Convertir a formato "largo" (long format) para ggplot
  pivot_longer(
    cols = everything(),
    names_to = "Grupo",
...
cat("Se han simulado", n_students, "estudiantes para cada grupo.\n")

# --- 4. Comparación Visual (Estética Renovada y Corregida) ---
ggplot(simulation_df, aes(x = Puntuacion, fill = Grupo)) +
  geom_density(alpha = 0.8, color = "white", linewidth = 0.1) +
  scale_fill_manual(values = c("Grupo A" = "#D55E00", "Grupo B" = "#0072B2")) +
  
  # --- CORRECCIÓN AQUÍ ---
  # Se usa paste() para crear un string y se añade parse = TRUE
  annotate("text", x = mean_A, y = 0.05, 
           label = paste("bold('Media A:') ~", mean_A), # 1. Crear el string
           parse = TRUE,                               # 2. Añadir parse
           color = "#D55E00", size = 4) +
  
  annotate("text", x = mean_B, y = 0.05, 
           label = paste("bold('Media B:') ~", mean_B), # 1. Crear el string
           parse = TRUE,                               # 2. Añadir parse
           color = "#0072B2", size = 4) +
  # --- FIN DE LA CORRECCIÓN ---
  
  labs(
    title = "Comparación de Puntuaciones: Grupo A vs. Grupo B",
    subtitle = bquote("Simulación de" ~ N(.(mean_A), .(sd_A)^2) ~ "y" ~ N(.(mean_B), .(sd_B)^2)),
    x = "Puntuación del Examen",
    y = "Densidad",
    fill = "Grupos Simulados"
  ) +
  theme_classic() +
  theme(
